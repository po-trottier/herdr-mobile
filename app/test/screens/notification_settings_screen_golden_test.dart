...
void main() {
  ...
  for (final testCase in _cases) {
    for (final (themeName, brightness) in _themes) {
      testWidgets(
        '${testCase.name} ($themeName) matches docs/31-mockups/12-notifications.md',
        (tester) async {
          ...
          await expectLater(
            ...
            matchesGoldenFile(
              'goldens/notification_settings_${testCase.name}_$themeName.png',
            ),
          );
          ...
          await tester.scrollUntilVisible(find.text('Send a test alert'), 200);
